...
// src/webpack/index.cjs
var path = require("node:path");
var PLUGIN_NAME = "BundlessWebpackPlugin";
var loaderPath = path.join(__dirname, "bundless.webpack-loader.cjs");
var RUNTIME_ALIASES = {
  bundlessdev$: false,
...
var BundlessWebpackPlugin = class {
  constructor(options = {}) {
    this.options = {
...
function getHtmlWebpackPluginConstructor(compiler) {
  return (compiler.options.plugins || []).map((plugin) => plugin && plugin.constructor).find(
    (constructor) => constructor && typeof constructor.getHooks === "function"
  );
}
